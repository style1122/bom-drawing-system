...
- **日志编码（两路不同，有意为之）**：`CONSOLE` appender 输出 **GBK**，匹配中文 Windows `cmd` 代码页 cp936——命令行窗口与 WinSW 捕获的 `bom-drawing-system.out.log` 用记事本直接打开即中文正常；`FILE` appender（`logs/bom-system.log`）保持 **UTF-8**，用 **VS Code / Notepad++ / 现代记事本**打开即正常。若希望命令行窗口也走 UTF-8，改用 `start.bat`（内置 `chcp 65001`）或 Windows Terminal，并将 `backend/src/main/resources/logback-spring.xml` 的 CONSOLE charset 改回 UTF-8 即可。
